...
def run_stage_profile(
        ...
        with output_records_jsonl.open("r", encoding="utf-8", errors="replace") as f:
            for line in f:
                ...
                try:
                    rec = json.loads(line)
                    rid = record_repo_id(rec)
                    prof = rec.get("llm_profile") or {}
                    # Only treat as completed if it was generated by a real model (not a fallback)
                    if (
                        rid
                        and prof
                        and not str(prof.get("profile_model", "")).endswith("-fallback")
                    ):
                        existing_profiled.add(rid.lower())
                except Exception:
                    pass
        print(
            f"[*] Stage 2 (Profile): Resumed {len(existing_profiled):,} real profiled records from {output_records_jsonl}"
        )
    ...
    print(f"[*] Using LLM model: {llm_cfg.model} @ {llm_cfg.base_url}")
    lock = threading.Lock()
        ...
        for attempt in range(4):
            try:
                ...
                if attempt < 3:
                    ...
                    continue
                # On permanent failure after 4 retries, attach fallback basic profile
